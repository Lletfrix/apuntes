...
\begin{pro}[Separabilidad de extensiones intermedias]
...
\begin{ex}[H3.18]
    ...
    \begin{itemize}
        ...
        % Además $K = \F_p(f) / \F_p$ es un cuerpo de escisión por lo que es una extensión normal. Como $g$ tiene una raíz en $K$ que es extensión normal $\implies$ $g$ se escinde en $K$. Por tanto:
        % $$
        %     g(x) = \prod_{i=1}^n (x-\alpha_i)
        % $$
        % y además:
        % $$
        %     f(x) = g \cdot \prod_{n+1}^q (x-\alpha_i)
        % $$
        % Como tenemos un algoritmo de la división en $\F_p[x]$, lo que nos dice que $\prod_{n+1}^q (x-\alpha_i) \in \F_p[x]$.

...
\begin{eg}[Hallar grupo de Galois dado un polinomio y una extensión]
    Sea $f \in \Q[x]$, $f(x) = x^4 - 2$. Y sea $E = \Q(\sqrt[4] 2)$, entonces el conjunto $\Omega$ del teorema \ref{thm:3.11} es $\Omega = \sdf{\sqrt[4] 2, -\sqrt[4] 2}$. Sea $L = \Q(\Omega) = E$, por el teorema \ref{3.11}(d), $\gal(E/\Q) \leq S_\Omega \isom S_2 \implies |\gal(E/\Q)| = 1$ o $2$ por el teorema de Lagrange de grupos. Además:
    $$
...
\begin{thm}[Derivada formal y raíces multiples]\label{thm:3.13}\label{thm:3.4.1}
    Si $f \in K[x]$ irreducible y $f'$ su derivada formal tal que $f' \neq 0$, entonces todas las raíces de $f$ en su cuerpo de escisión son distintas.
\end{thm}
\begin{obs}
...
\begin{pro}[Multiplicidad de raíces y derivada formal]\ref{pro:3.4.2}
...
\begin{pro}[Divisores irreducibles de un polinomio separable]
...
\begin{lm}[Separabilidad en cuerpos de característica $0$]\ref{lm:3.4.3}
...
\begin{cor}[Separabilidad en cuerpos de característica $p$]\ref{cor:3.4.4}
...
\begin{ex}[H3.16]
    ¿Cuántas raíces distitnas tiene $x^{12} + 2x^6 + 1 \in \F_3[x]$ en su cuerpo de escisión?\\\\

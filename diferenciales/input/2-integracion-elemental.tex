...
\begin{dfn}[Ecuación homogénea de grado k]
    Sea $f: \mathbb{K} x \mathbb{K} \longrightarrow \mathbb{K}$, decimos que es \textbf{homogénea de grado k} $\iff f(\lambda x_1, \lambda x_2) = \lambda^k \cdot f(x_1, x_2)$
\end{dfn}
...
En la sección anterior hemos enunciado una proposición que denominamos de \textit{existencia y unicidad} para ecuaciones lineales de orden I. Nos gustaría dar condiciones más generales para saber si existen y son únicas ciertas soluciones.\\
...
\end{thm}
La demostración la veremos más adelante cuando consideremos el caso \textit{n-dimensional}.
...
\end{proof}
\begin{obs}
    Sabemos que una ecuación es exacta cuando $(M, N) = \nabla g$, sea $\gamma$ cualquier curva tal que $\gamma : [a,b]\subset\R \longrightarrow \R^2$ en $\R^2$. Entonces:
    $$
...
\begin{eg}[Factores integrantes]
    Sea $x\mbf{y}\ \d x + \mbf{y}^2 \d y = 0$, esta claro que $M = x\mbf{y}$, $N = \mbf{y}^2$. Para ver si puede ser una ecuación exacta evaluamos si cumple la condición necesaria $M_y = N_x$, sin embargo las derivadas cruzadas son $M_y=x$ y $N_x = 0$ y por tanto no puede ser ecuación exacta.\\\\
...
\begin{eg}[Muelles]\label{eg:muelle}
    ...
    \begin{enumerate}
        ...
        $$
            \exists \alpha, \beta : \mbf{y}_{\alpha, \beta} = \alpha \sin (t) + \beta \cos (t)
        $$
        que cumplen:
        $$

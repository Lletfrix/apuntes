...
\section{Funciones complejas}
    ...
    \begin{pro}[Operaciones de funciones continuas]
    ...
    \begin{dfn}[Convergencia uniforme]
    ...
    \begin{pro}[Convergencia uniforme. Condición necesaria y suficiente]
    ...
    \begin{eg}[Aplicación del M-test]
...
\begin{dfn}[Derivabilidad. Función holomorfa]
...
\begin{eg}[Derivabilidad de funciones]$ $
    \begin{enumerate}[1)]
        \item Sea\\
        \fn{$f$}{$\C$}{$\C$}{$z$}{$\cnj{z}$}
...
\begin{pro}[Derivada de suma y producto en $\C$]
...
\begin{pro}[Derivabilidad y continuidad]
...
Consideremos $z\in \C:\ z = x+iy$ con $x,y\in \R$. También $f : D \subseteq \C \to \C$ una función derivable (en el sentido complejo), que podemos interpretar como función $f: D\subseteq \R^2 \to \R^2$, es decir, $f(z) = f(x, y) = u(x, y) + i v(x, y)$, con $u,v: D \subseteq \R^2 \to \R$. Entonces:
$$
...
\begin{pro}[Ecuaciones de Cauchy-Riemann]
...
\begin{pro}[Derivada de la función inversa]
...
\begin{pro}[Función holomorfa constante (1)]
...
\begin{pro}[Función holomorfa constante (2)]
